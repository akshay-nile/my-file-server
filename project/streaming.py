import os
import io
from project.utilities import format_url, join_path
...
def upload(req):
    try:
        savepath = f"C:/Users/{os.getenv('USERNAME')}/Downloads"
...
def thumbnail(data, thumbname, root):
    ...
    thumbimg.save(join_path(root, 'static', 'images', 'thumbnails', thumbname))
    buffer.close()
...
def thumbnails(req, app):
    ...
    for line in req.get_data().decode('utf-8').split('\n'):
        ...
        if os.path.exists(join_path(root, 'static', 'images', 'thumbnails', thumbname)):
        ...
        elif os.path.exists(join_path(root, 'static', 'images', 'default', extension + '.png')):
            thumbpath = f'/static/images/default/{extension}.png'
